...
public class MenuManagementWebService {
    ...
    public ArrayList<String> getAllMenu() {
        ...
        for (Object obj : listMenu) {
            Menu m = (Menu)obj;
            String baris = m.getId() + "|" + m.getNama() + "|" + m.getHarga() + "|" + m.getKeterangan() + "|" + m.getKategori() + "|" + m.getStatus();
    ...
    public String addMenu(@WebParam(name="nama") String nama, @WebParam(name="harga") int harga, @WebParam(name="keterangan") String keterangan, @WebParam(name="kategori") String kategori, @WebParam(name="status") String status) {
        try {
            Menu menu = new Menu();
            if (menu.isNamaExist(nama, 0)) {
                return "FAILED|Nama menu sudah terdaftar. Silakan gunakan nama lain!";
            }
            menu.setNama(nama);
            menu.setHarga(harga);
            menu.setKeterangan(keterangan);
    ...
    public String updateMenu(@WebParam(name="id") int id, @WebParam(name="nama") String nama, @WebParam(name="harga") int harga, @WebParam(name="keterangan") String keterangan, @WebParam(name="kategori") String kategori, @WebParam(name="status") String status) {
        try {
            Menu menu = new Menu();
            if (menu.isNamaExist(nama, id)) {
                return "FAILED|Nama menu sudah terdaftar. Silakan gunakan nama lain!";
            }
            menu.setId(id);
            menu.setNama(nama);
